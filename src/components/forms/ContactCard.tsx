...
export default function ContactCard({ onSuccess }: Props) {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (!name.trim() || !email.trim() || !message.trim()) { setError("Please fill in all required fields."); return; }
    setLoading(true);
    setTimeout(() => { setLoading(false); setSent(true); onSuccess?.(); }, 1000);
  };
  ...
  if (sent) return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center">
      ...
      <h3 className="font-extrabold text-gray-800 mb-1">Message sent!</h3>
      <p className="text-sm text-gray-500">We'll get back to you within 24 hours at <strong>{email}</strong>.</p>
    </div>
  );
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-4">
        <p className="text-white font-extrabold text-sm">📬 Contact ISA Support</p>
        <p className="text-blue-200 text-[10px] mt-0.5">We typically reply within 24 hours</p>
      </div>
