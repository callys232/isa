      ...
      <div className="p-4">
        ...
        {step.action && (
          <Link href={step.action.href}
            className="inline-flex items-center gap-1 text-[11px] font-bold px-3 py-1.5
              bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all active:scale-95">
            {step.action.label} →
          </Link>
        )}
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50">
          <button disabled={idx === 0} onClick={() => setIdx(i => i - 1)}
            className="px-3 py-1.5 text-xs font-bold text-gray-500 hover:text-gray-700 disabled:opacity-30 transition-colors active:scale-95">
            ← Prev
          </button>
            ...
            {steps.map((_, i) => (
              <button key={i} onClick={() => setIdx(i)}
                className={`h-2 rounded-full transition-all duration-200
                  ${i === idx ? "bg-green-600 w-4" : "bg-gray-300 hover:bg-gray-400 w-2"}`} />
            ))}
          </div>
